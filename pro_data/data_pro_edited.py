...
def review_normalize(df): #1~5 to 0~1
    df['rating'] = (df['rating']-1)/4

def column_rename(df): #"reviewText"->"reviews"
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 2 and sys.argv[2] == 'yelp':
        ...
        # amazon dataset
        save_folder = '../dataset/' + filename+"_data"
    ...
    folderpath = "amazon_"+filename+"/"
    train_input = pd.read_csv(os.path.join(folderpath,filename+"_core_train.csv"),sep=",")
    valid_input = pd.read_csv(os.path.join(folderpath,filename+"_core_valid.csv"),sep=",")
    test_input = pd.read_csv(os.path.join(folderpath,filename+"_core_test.csv"),sep=",")
    

    review_normalize(train_input)
    review_normalize(valid_input)
    review_normalize(test_input)
    column_rename(train_input)
    column_rename(valid_input)
    column_rename(test_input)

    full_input = pd.concat([train_input, valid_input,test_input]) 
    ...
    if yelp_data:
        ...
        users_id = full_input['user_id'].tolist()
    ...
    uid_index = []
    for uid in uidMiss:
        index = data_valid_plus_test.index[data_valid_plus_test['user_id'] == uid].tolist()
        uid_index.extend(index)
    data_train = pd.concat([data_train, data_valid_plus_test.loc[uid_index]])

    iid_index = []
    for iid in iidMiss:
        index = data_valid_plus_test.index[data_valid_plus_test['item_id'] == iid].tolist()
        iid_index.extend(index)
    data_train = pd.concat([data_train, data_valid_plus_test.loc[iid_index]])
